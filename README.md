...
-   `rename = "NewName"` names the generated TypeScript declaration. References do not follow it yet — point them at the new name with `#[tsify(type = "NewName")]` where another type refers to this one ([#94](https://github.com/madonoharu/tsify/issues/94) tracks making that automatic). Cannot be combined with `type_prefix` or `type_suffix`.
